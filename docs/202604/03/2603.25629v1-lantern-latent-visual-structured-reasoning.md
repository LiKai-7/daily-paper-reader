...
score: 7.0
evidence: 多模态模型中的潜空间视觉推理
tldr: 针对大型多模态模型在视觉推理中过度依赖文本描述而导致空间理解不足的问题，本文提出LanteRn框架。该框架允许模型在推理过程中交替生成语言和紧凑的潜空间视觉表示，直接在潜空间进行视觉推理。通过监督微调和强化学习两阶段训练，LanteRn在多个感知基准测试中显著提升了视觉定位和细粒度推理能力，为高效多模态推理提供了新方向。
source: arxiv
...
motivation: 现有模型将视觉内容转化为文本进行推理，限制了对细粒度空间和视觉信息的理解效率与精度。
method: 提出一种在潜空间进行视觉推理的框架，通过生成和关注连续的视觉思维嵌入，并结合监督微调与强化学习进行训练。
result: "在VisCoT、V*和Blink等感知中心基准测试中，该方法在视觉定位和细粒度推理方面表现出持续的性能提升。"
conclusion: 内部潜空间表示是实现更高效、更精准的多模态结构化推理的一种极具前景的方向。
---

## 摘要
尽管语言推理模型在许多任务中表现出色，但视觉推理对于当前的大型多模态模型（LMMs）而言仍然具有挑战性。因此，大多数 LMM 默认将感知内容转化为文本，这对于需要细粒度空间和视觉理解的任务来说是一个严重的局限。虽然最近的一些方法通过调用工具或生成中间图像来尝试“以图思考”，但它们要么依赖外部模块，要么因直接在像素空间进行推理而产生不必要的计算开销。在本文中，我们介绍了 LanteRn，这是一个允许 LMM 将语言与紧凑的潜空间视觉表示交织在一起的框架，从而使视觉推理能够直接在潜空间中进行。LanteRn 增强了视觉-语言 Transformer 的能力，使其在推理过程中能够生成并关注连续的视觉思维嵌入。我们分两个阶段训练该模型：首先通过监督微调将视觉特征锚定在潜状态中，随后通过强化学习使潜空间推理与任务级效用保持一致。我们在三个以感知为中心的基准测试（VisCoT、V* 和 Blink）上评估了 LanteRn，观察到其在视觉定位和细粒度推理方面均有持续提升。这些结果表明，内部潜空间表示为更高效的多模态推理提供了一个极具前景的方向。
