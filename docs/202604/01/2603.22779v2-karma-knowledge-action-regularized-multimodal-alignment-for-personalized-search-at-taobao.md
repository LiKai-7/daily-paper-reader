...
title_zh: KARMA：淘宝个性化搜索中的知识-行为正则化多模态对齐
...
tldr: "针对淘宝个性化搜索中大模型微调导致的“知识-行动鸿沟”和语义坍缩问题，本文提出KARMA框架。该框架将语义重构作为训练时的正则化项，通过历史条件语义生成和嵌入条件语义重构，在优化搜索行动目标的同时保持LLM原有的语义解码能力。实验证明KARMA能有效缓解语义退化，显著提升了淘宝搜索在召回、粗排和精排阶段的性能，并实现线上GMV增长0.9%。"
source: arxiv
